public class Entity1 extends Entity
{
    ...
    public Entity1()
    {
        System.out.println("Initializing entity 1 at time: " + NetworkSimulator.time);
        ...
        while(i < 4)
        {
            ...
            Packet packet = new Packet(1, i, distanceTable[1]);
            System.out.println("Sending first distance vector from 1 to " + i + " at " + NetworkSimulator.time);
    ...
    {
        System.out.println("Packet received in 1 via " + p.getSource() + " at time: " + NetworkSimulator.time);
        int myDistanceVector [] = distanceTable[1];
        int neighbors [] = NetworkSimulator.cost[1];
        int i = 0;
        ...
        if(vectorChanged)
        {
            ...
            while(i < 4)
            {
                distanceTable[1][i] = myDistanceVector[i];
                i++;
            }
            // if the vector was changed, send it to neighbors
            System.out.println("Distance vector changed in 1");
            toStringVector();
            printDT();
            i = 0;
            while(i < 4)
            {
                ...
                Packet packet = new Packet(1, i, distanceTable[1]);
                System.out.println("Sending distance vector from 1 to " + i + " at: " + NetworkSimulator.time);
                NetworkSimulator.toLayer2(packet);
                i++;
            }
        }
        ...
        {
            System.out.println("The distance vector in 1 was not updated");
            printDT();
        }
        System.out.println();
    }
    ...
    {
        System.out.println("Link cost change detected in 1 for : Link " + whichLink +
        ...
        if(vectorChanged)
        {
            ...
            while(i < 4)
            {
                ...
                System.out.println("Sending distance vector from 1 to " + i + " at: " + NetworkSimulator.time);
                NetworkSimulator.toLayer2(packet);
                System.out.println("Sending packet from 1 to " + i + " in init");
                i++;
            }
        }
        else
    ...
    {
        System.out.println("Updated distance vector for 1:");
        System.out.println("[" + distanceTable[1][0] + ", " + distanceTable[1][1] + ", " + distanceTable[1][2] + ", " + distanceTable[1][3] + "]");
    }
}
