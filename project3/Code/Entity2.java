public class Entity2 extends Entity
{
    ...
    public Entity2()
    {
        System.out.println("Initializing entity 2 at time: " + NetworkSimulator.time);
        ...
        while(i < 4)
        {
            ...
            Packet packet = new Packet(2, i, distanceTable[2]);
            System.out.println("Sending first distance vector from 2 to " + i + " at " + NetworkSimulator.time);
    ...
    {
        System.out.println("Packet received in 2 via " + p.getSource() + " at time: " + NetworkSimulator.time);
        int myDistanceVector [] = distanceTable[2];
        int neighbors [] = NetworkSimulator.cost[2];
        int i = 0;
        ...
        if(vectorChanged)
        {
            ...
            while(i < 4)
            {
                distanceTable[2][i] = myDistanceVector[i];
                i++;
            }
            // if the vector was changed, send it to neighbors
            System.out.println("Distance vector changed in 2");
            toStringVector();
            printDT();
            i = 0;
            while(i < 4)
            {
                ...
                Packet packet = new Packet(2, i, distanceTable[2]);
                System.out.println("Sending distance vector from 2 to " + i + " at: " + NetworkSimulator.time);
                NetworkSimulator.toLayer2(packet);
                i++;
            }
        }
        ...
        {
            System.out.println("The distance vector in 2 was not updated");
            printDT();
        }
        System.out.println();
    }
    ...
    {
        System.out.println("Updated distance vector for 2:");
        System.out.println("[" + distanceTable[2][0] + ", " + distanceTable[2][1] + ", " + distanceTable[2][2] + ", " + distanceTable[2][3] + "]");
    }
}
